...
public class SVMRecognition {
    ...
    public static void testingTheClassifier(Instances testingInstances) {
        try {
            ...
            Classifier classifier = MainActivity.getClassifierSVMH();
    ...
    public void init() throws Exception {
       ...
       try {
            classifier = MainActivity.getClassifierSVMH();
        } catch (Exception ex) {
            System.err.println("Classifier not found. Training a new classifier for gesture(Horizontal) recognition");
        }
        if(classifier==null)
        {
            System.err.println("Please add the Horizontal Model to the model folder of the profile");
        }
